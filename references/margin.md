...
| View or change leverage for a pair | Leverage Management | *(see `{baseDir}/references/futures.md`)* |
...
`PUT /v1/margin/account/status` — see `{baseDir}/references/subaccounts.md` for the
complete workflow.
...
3. Futures can be enabled on a subaccount via the API using
   ...
   show them the API path. See `{baseDir}/references/subaccounts.md` for the full workflow.
...
via `PUT /v1/margin/account/status`. See `{baseDir}/references/subaccounts.md` for the
...
```bash
python3 {baseDir}/scripts/valr_request.py GET /v2/margin/status \
  --subaccount-id YOUR_SUBACCOUNT_ID
```
...
**Leverage Configuration** section in `{baseDir}/references/futures.md`. The relevant
endpoints are:
